...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length != 1){
			...
			//发送ajax请求，生成未关联客户列表
			var url_1="DecidedzoneAction_findNotAssociation";
			$.post(url_1,function(data){
				for(var i=0;i<data.length;i++){
					var id=data[i].id;
					var name=data[i].name;
					var phone=data[i].telephone;
					$("#noassociationSelect").append("<option value='"+id+"'>"+name+"("+phone+")"+"</option>");
				}
			});
			
			$("#associationSelect").html("");
			var id=rows[0].id;
			var url_2="DecidedzoneAction_findHasAssociation";
			$.post(url_2,{"id":id},function(data){
				for(var i=0;i<data.length;i++){
					var id=data[i].id;
					var name=data[i].name;
					var phone=data[i].telephone;
					$("#associationSelect").append("<option value='"+id+"'>"+name+"("+phone+")"+"</option>");
				}
			});
		}
	}
	...
	function doDblClickRow(index,data){
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "SubareaAction_relationDecide?decidedzoneId="+data.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "DecidedzoneAction_findHasAssociation?id="+data.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="addForm" method="post" action="DecidedzoneAction_add">
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"  
    							data-options="valueField:'id',textField:'name',url:'StaffAction_select'" />  
						</td>
					</tr>
					<tr height="300">
						<td valign="top">关联分区</td>
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px" data-options="url:'SubareaAction_selectNotGive',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
	...
	<div modal="true" class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 400px;height: 300px;">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="customerForm" action="${pageContext.request.contextPath }/DecidedzoneAction_assigncustomer" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
							...
							<script type="text/javascript">
								$(function(){
									...
									$("#toRight").click(function(){
										//append也可以实现剪切复制的操作，将选中的选项添加到另一个对象中，原对象就会删除该选项
