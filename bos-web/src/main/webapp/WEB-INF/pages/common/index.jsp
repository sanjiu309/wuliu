...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btnEp").click(function(){
			//进行表单校验
			var bool=$("#modifyPwd").form("validate");
			if(bool){
				//表单校验通过
				//手动校验两次输入是否一致
				...
				if(pwd1 == pwd2){
					//两次输入一致
					$.post("UserAction_modifyPwd",{"password":pwd1},function(data){
						if(data == '1'){
							//修改成功，关闭修改密码窗口
					...
					<table cellpadding=3>
						<tr>
							...
							<td><input required="required"
								data-options="validType:'length[4,20]'" id="txtNewPass"
								type="Password" class="txt01 easyui-validatebox" /></td>
						</tr>
						<tr>
							<td>确认密码：</td>
							<td><input required="required"
								data-options="validType:'length[4,20]'" id="txtRePass"
								type="Password" class="txt01 easyui-validatebox" /></td>
						</tr>
					</table>
				</form>
